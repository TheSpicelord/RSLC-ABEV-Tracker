...
- **`STATE_MODELS` and `ACTIVE_STATES` are separate questions.** A state needs both a model *and* rows in `dbo.General_Absentees_2026` before it can be published. As of 2026-09-01 the feed carries VA, PA, NJ, WI, IL, MN, GA, AK, RI: **PA/NJ/GA are wired and ready to activate**; **IL/MN** have data but no model (they would work on the national fallback, like RI); **NV/AZ/MI/TX/IA/OR** have models but no feed rows yet. Adding a state with no feed rows produces an empty state, not an error.
