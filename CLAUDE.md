...
- **Retired district lines** (`HISTORY_STALE_LINES` in config.js, year → state abbrs; currently `2022: ["VA", "WI", "NC"]`; **NC's 2022 ran on the court-supervised interim maps, SL 2022-2 Senate / SL 2022-4 House, which the General Assembly replaced 2023-10-25 with SL 2023-146 / SL 2023-149 — the lines used in 2024 and again in 2026, so NC's 2024 ABEV is comparable and its 2022 is not. The NC constitution allows only one legislative redraw per census, so the Oct 2025 mid-decade redistricting was congressional-only.** District Explorer's `leg_2022` for NC was dropped to match on 2026-09-05 — see `LEG_REDISTRICTED` in its `generate_chamber_jsons.py` — so NC now renders exactly like WI: one real leg column (2024), 2022 as an explicit N/A column, and the footnote. **These two lists have to move together**: dropping a year in DE without a `LEG_REDISTRICTING_NOTES` entry here makes it vanish unexplained, and adding one here without dropping it there shows a live 2022 margin beside an N/A 2022 ABEV column): the year's data is still pulled and loaded, but the UI refuses to show it. `historyYearAppliesToSelectedState(year)` marks those column defs `na`, so both the count and margin cells render "N/A" (`historyNaCellHtml`) in every table, that sort key sorts as blank, and `trendYearsForScope()` drops the year so no checkbox or line appears. The columns stay in place on purpose — the years line up with every other state. `legRedistrictingNote()` surfaces the state's `LEG_REDISTRICTING_NOTES` footnote when either the leg-margin column or the past-cycle ABEV column is N/A.
